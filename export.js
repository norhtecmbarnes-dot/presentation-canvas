(function () {
    ...
    function parseSlideHtml(html) {
        ...
        if (bodyEl) {
            const bodyStyle = bodyEl.style;
            if (bodyStyle.background) bgColor = parseCssColor(bodyStyle.background);
            if (bodyStyle.backgroundColor) bgColor = parseCssColor(bodyStyle.backgroundColor);
            if (bodyStyle.color) textColor = parseCssColor(bodyStyle.color);
        }

        const styleTag = doc.querySelector('style');
        if (styleTag) {
            const css = styleTag.textContent;
            const bodyBgMatch = css.match(/body\s*\{[^}]*background(?:-color)?\s*:\s*([^;}]+)/);
            if (bodyBgMatch && !bgColor) bgColor = parseCssColor(bodyBgMatch[1].trim());
            ...
            if (!bgColor && cssVars.bg) bgColor = parseCssColor(cssVars.bg);
            if (!textColor && cssVars.text) textColor = parseCssColor(cssVars.text);

            const bodyColorMatch = css.match(/body\s*\{[^}]*color\s*:\s*([^;}]+)/);
            if (bodyColorMatch && !textColor) textColor = parseCssColor(bodyColorMatch[1].trim());
        }
        ...
        const texts = [];
        headings.forEach(h => {
            const fontSize = getComputedFontSize(h);
            texts.push({
                type: 'title',
                text: h.textContent.trim(),
                fontSize: fontSize,
                bold: true,
                color: getElementColor(h) || textColor
            });
        });

        const paragraphs = doc.querySelectorAll('p');
        paragraphs.forEach(p => {
            const isHeading = p.querySelector('h1,h2,h3,h4,h5,h6');
            if (isHeading) return;
            texts.push({
                type: 'body',
                text: p.textContent.trim(),
                fontSize: getComputedFontSize(p),
                bold: false,
                color: getElementColor(p) || textColor
            });
        });
        ...
        listItems.forEach(li => {
            texts.push({
                ...
                fontSize: getComputedFontSize(li),
                bold: false,
                color: getElementColor(li) || textColor
            });
        });
        ...
        return {
            title,
            texts: texts.filter(t => t.text.length > 0),
            bgColor,
            textColor,
            images: imageData
        };
    ...
    function parseCssColor(str) {
        ...
        if (namedColors[str.toLowerCase()]) return namedColors[str.toLowerCase()];
    ...
    function getElementColor(el) {
        const style = el.getAttribute('style') || '';
        const match = style.match(/color\s*:\s*([^;}"\s]+)/);
        if (match) return parseCssColor(match[1]);
        return null;
    }
...
<p:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="${SLIDE_WIDTH_EMU}" cy="${SLIDE_HEIGHT_EMU}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:solidFill><a:srgbClr val="${slide.bgColor}"/></a:solidFill><a:ln><a:noFill/></a:ln></p:spPr>
            ...
            const hasTitle = slide.texts.some(t => t.type === 'title');
            let currentY = 457200;
            let shapeId = slide.bgColor ? 2 : 1;

            if (hasTitle) {
                const titleTexts = slide.texts.filter(t => t.type === 'title');
                const titleColor = titleTexts[0]?.color || (slide.bgColor && isLightColor(slide.bgColor) ? '000000' : 'FFFFFF');
                const titleSize = titleTexts[0]?.fontSize || 44;

                let paras = '';
                titleTexts.forEach(t => {
                    paras += `<a:p><a:r><a:rPr lang="en-US" sz="${titleSize * 100}" b="1" dirty="0"><a:solidFill><a:srgbClr val="${t.color || titleColor}"/></a:solidFill><a:latin typeface="Calibri Light"/></a:rPr><a:t>${escapeXml(t.text)}</a:t></a:r></a:p>`;
                });
...
<p:spPr><a:xfrm><a:off x="457200" y="${currentY}"/><a:ext cx="${SLIDE_WIDTH_EMU - 914400}" cy="${Math.round(titleSize * 1.5 * EMU_PER_INCH / 10)}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr>
<p:txBody><a:bodyPr anchor="b"/><a:lstStyle/>${paras}</p:txBody>
</p:sp>`;

                currentY += Math.round(titleSize * 1.5 * EMU_PER_INCH / 10) + 114300;
                shapeId++;
            }

            const bodyTexts = slide.texts.filter(t => t.type !== 'title');
            if (bodyTexts.length > 0) {
                const bodyColor = bodyTexts[0]?.color || (slide.bgColor && isLightColor(slide.bgColor) ? '000000' : 'FFFFFF');

                let paras = '';
                bodyTexts.forEach(t => {
                    const sz = t.fontSize ? t.fontSize * 100 : 1800;
                    const isBullet = t.type === 'bullet';
                    const bulletProps = isBullet ? '<a:buChar char="&#x2022;"/>' : '';
                    const tColor = t.color || bodyColor;
                    paras += `<a:p>${bulletProps}<a:r><a:rPr lang="en-US" sz="${sz}" dirty="0"><a:solidFill><a:srgbClr val="${tColor}"/></a:solidFill><a:latin typeface="Calibri"/></a:rPr><a:t>${escapeXml(t.text)}</a:t></a:r></a:p>`;
                });

                const bodyHeight = SLIDE_HEIGHT_EMU - currentY - 457200;
...
<p:spPr><a:xfrm><a:off x="${imgX}" y="${imgY}"/><a:ext cx="${imgWidth}" cy="${imgHeight}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr>
                ...
                imageParts.push({
                    name: `ppt/media/image${imageCounter}.${img.ext}`,
                    content: bytes
                });
...
<a:gradFill rotWithShape="1"><a:gsLst><a:gs pos="0"><a:schemeClr val="phClr"><a:shade val="51000"/><a:satMod val="130000"/></a:schemeClr></a:gs><a:gs pos="100000"><a:schemeClr val="phClr"><a:shade val="94000"/><a:satMod val="135000"/></a:schemeClr></a:gs></a:gsLst><a:lin ang="16200000" scaled="0"/></a:gradFill>
</a:fillStyleLst>
<a:lnStyleLst>
...
<a:effectStyle><a:effectLst/></a:effectStyle>
<a:effectStyle><a:effectLst><a:outerShdw rotWithShape="1" blurRad="40000" dist="23000" dir="5400000" rotWithShape="0"><a:srgbClr val="000000"><a:alpha val="35000"/></a:srgbClr></a:outerShdw></a:effectLst></a:effectStyle>
</a:effectStyleLst>
<a:bgFillStyleLst>
<a:solidFill><a:schemeClr val="phClr"/></a:solidFill>
<a:gradFill rotWithShape="1"><a:gsLst><a:gs pos="0"><a:schemeClr val="phClr"><a:tint val="40000"/><a:satMod val="350000"/></a:schemeClr></a:gs><a:gs pos="100000"><a:schemeClr val="phClr"><a:tint val="100000"/><a:satMod val="350000"/></a:schemeClr></a:gs></a:gsLst><a:lin ang="16200000" scaled="1"/></a:gradFill>
<a:gradFill rotWithShape="1"><a:gsLst><a:gs pos="0"><a:schemeClr val="phClr"><a:shade val="51000"/><a:satMod val="130000"/></a:schemeClr></a:gs><a:gs pos="100000"><a:schemeClr val="phClr"><a:shade val="94000"/><a:satMod val="135000"/></a:schemeClr></a:gs></a:gsLst><a:lin ang="16200000" scaled="0"/></a:gradFill>
</a:bgFillStyleLst>
</a:fmtScheme>
</a:themeElements>
